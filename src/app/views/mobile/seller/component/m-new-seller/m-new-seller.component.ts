...
@Component({
  selector: 'app-m-new-seller',
  templateUrl: './m-new-seller.component.html',
  styleUrls: ['./m-new-seller.component.scss']
})
export class MNewSellerComponent implements OnInit {
  ...
  private user = JSON.parse(localStorage.user);
    ...
    cekStore() {
      this.storeService.getAll().subscribe(response => {
        if (response.length) {
          this.routes.navigateByUrl('/mobile-seller');
        }
      });
    }
    ...
    changeData() {
      this.routes.navigateByUrl('/mobile-seller/m-profile');
    }
